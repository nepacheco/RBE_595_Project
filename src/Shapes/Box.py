...
class Box(Shape):
    ...
    def planGrasps(self, graspParams, surfaceOffset = 100):
        ...
        for i in range(numberCubeSides):
            ...
            # Generate parallel matrix representing the graspers frame
            translationMatrix = Pose.makeTranformfromPose(Pose(xAxisValue/2, 0, -zEdgeDistance, 0, 0, 0))
            ...
            for j in range(1, parallelPlanes+1):  # Vertical and Horizontal
                for k in range(1, parallelPlanes+1):
                    ...
                    transXMatrix = Pose.makeTranformfromPose(Pose(-k*(xAxisValue/(parallelPlanes+1)), 0, 0, 0, 0, 0))
                    transZMatrix = Pose.makeTranformfromPose(Pose(0, 0, j*(zAxisValue/(parallelPlanes+1)), 0, 0, 0))
